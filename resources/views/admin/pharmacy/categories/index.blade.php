    ...
    <div class="content">
        ...
        <div class="block">
            <div class="block-header">
                <h3 class="block-title">{{__('pharmacy.category_list')}}</h3>
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            <a class="link-fx" href="">{{__('pharmacy.category_list')}}</a>
                        </li>
                    </ol>
                </nav>
            </div>
            <div class="block-header">
                @can('ph_donor_create')
                    <a href="{{route('admin.pharmacy.categories.create')}}" class="btn btn-sm btn-primary">{{__('general.add_new')}}</a>
                @endcan
                ...
                @can('ph_donor_delete')
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu font-size-sm primary" aria-labelledby="dropdown-default-primary">
                            <a class="dropdown-item" href="{{ route('admin.pharmacy.categories.index') }}?show_all=1"><i class="fa fa-list-alt"></i> {{__('general.show_all')}}</a>
                            <a class="dropdown-item" href="{{ route('admin.pharmacy.categories.index') }}?trash=1"><i class="fa fa-trash-alt"></i> {{__('general.trash')}}</a>
                        </div>
                    </div>
                @endcan
...
{{--                <table class="table table-bordered table-striped table-vcenter js-dataTable-buttons"--}}
                    ...
                    <thead>
                    <tr>
                        <th >{{__('pharmacy.category_name')}}</th>
                        <th >{{__('pharmacy.category_abbreviation')}}</th>
                        <th >{{__('pharmacy.category_description')}}</th>
                        <th >{{__('pharmacy.category_status')}}</th>
                        <th style="width: 15%;">{{__('pharmacy.category_action')}}</th>
                    </tr>
                    </thead>
                </table>
            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#datatable_ph_category').DataTable({
                ...
                // ],
                ajax: {
                    url: "{{route('admin.pharmacy.categories.index')}}{{ request('trash') == 1 ? '?trash=1':'' }}",
                },
                columns: [
                    {data: 'name', name: 'name'},
                    {data: 'abr', name: 'abr'},
            ...
            $('#ok_button').click(function () {
                $.ajax({
                    ...
                    url: "categories/{{ request('trash') == 1 ? 'per_del/':'' }}" + department_id,
                    type: 'DELETE',
                    ...
                    error: function () {
                        One.helpers('notify', {
                            ...
                            message: "{{__('pharmacy.category_delete_error')}}"
                        });
                    }
                })
            })
            ...
            $('#restore_button').click(function () {
                $.ajax({
                    ...
                    url: "categories/restore/" + department_id,
                    type: 'POST',
                    success: function (data) {
                    ...
                    error: function () {
                        One.helpers('notify', {
                            type: 'danger',
                            icon: 'fa fa-times mr-1',
                            message: "{{__('pharmacy.category_restore_error')}}"
                        });
                    }
                })
            })
